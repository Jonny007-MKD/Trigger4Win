...
namespace Trigger.Classes.WMI {
	...
	public class NetworkAdapter : Component
	{
		...
		/// </summary>
		internal static string CreatedClassName = "Win32_NetworkAdapter";
		...
		/// <summary>
		/// <para>LateBound <see cref="NetworkAdapterConfiguration"/></para>
		/// </summary>
		internal NetworkAdapterConfiguration networkAdapterConfiguration = null;
		#endregion
		...
		public NetworkAdapter(NetworkInterface networkInterface) : this(networkInterface.Id)
		{
		}
		...
		public NetworkAdapterConfiguration Configuration
		{
			get
			{
				if (networkAdapterConfiguration == null)
					return networkAdapterConfiguration = this.GetNetworkAdapterConfiguration();
				else
		...
		/// <summary>
		/// <para> Network medium in use. The network adapters are as follows:</para>
		/// <para>
		/// <list type="bullet">
		/// <item></item>
		/// <item></item>
...
.. - Andere - Informationen zu anderen als den oben aufgeführten ganzzahligen Werte finden Sie in der Win32-Fehlercodedokumentation.")]
		...
		internal NetworkAdapterConfiguration GetNetworkAdapterConfiguration()
		{
			NetworkAdapterConfiguration.NetworkAdapterConfigurationCollection configs = new NetworkAdapterConfiguration.NetworkAdapterConfigurationCollection(this.internalLateBoundObject.GetRelated(NetworkAdapterConfiguration.CreatedClassName));
			if (configs == null)
				return null;
			else
				return configs[0];
		...
		internal bool ShouldSerializeAdapterTypeId() {
		...
		internal bool ShouldSerializeAutoSense() {
		...
		internal bool ShouldSerializeAvailability() {
		...
		internal bool ShouldSerializeConfigManagerErrorCode() {
		...
		internal bool ShouldSerializeConfigManagerUserConfig() {
		...
		internal bool ShouldSerializeErrorCleared() {
		...
		internal bool ShouldSerializeInstallDate() {
		...
		internal bool ShouldSerializeInstalled() {
		...
		internal bool ShouldSerializeLastErrorCode() {
		...
		internal bool ShouldSerializeMaxNumberControlled() {
		...
		internal bool ShouldSerializeMaxSpeed() {
		...
		internal void ResetNetConnectionID() {
		...
		internal bool ShouldSerializeNetConnectionStatus() {
		...
		internal bool ShouldSerializeNetEnabled() {
		...
		internal bool ShouldSerializePhysicalAdapter() {
		...
		internal bool ShouldSerializePowerManagementSupported() {
		...
		internal bool ShouldSerializeSpeed() {
		...
		internal bool ShouldSerializeStatusInfo() {
		...
		internal bool ShouldSerializeTimeOfLastReset() {
		...
		internal static string ConstructPath(string keyDeviceID)
		{
		...
		/// <summary>
		/// <para>Disables the network adapter.</para>
		/// </summary>
		/// <returns>Returns zero (0) to indicate success. Any other number indicates an error.</returns>
		public WbemError Disable()
		{
			if ((isEmbedded == false)) {
				ManagementBaseObject inParams = null;
				ManagementBaseObject outParams = internalLateBoundObject.InvokeMethod("Disable", inParams, null);
				return (WbemError)Convert.ToUInt32(outParams.Properties["ReturnValue"].Value);
			}
			else {
				return WbemError.NoErr;
			}
		}
		
		/// <summary>
		/// <para>Enables the network adapter.</para>
		/// </summary>
		/// <returns>Returns zero (0) to indicate success. Any other number indicates an error.</returns>
		public WbemError Enable()
		{
			if ((isEmbedded == false)) {
				ManagementBaseObject inParams = null;
				ManagementBaseObject outParams = internalLateBoundObject.InvokeMethod("Enable", inParams, null);
				return (WbemError)Convert.ToUInt32(outParams.Properties["ReturnValue"].Value);
			}
			else {
				return WbemError.NoErr;
		...
		/// <summary>
		/// <para>Not implemented. For more information about how to implement this method, see the Reset method in CIM_NetworkAdapter.</para>
		/// </summary>
		/// <returns></returns>
		public WbemError Reset()
		{
			if ((isEmbedded == false)) {
				ManagementBaseObject inParams = null;
				ManagementBaseObject outParams = internalLateBoundObject.InvokeMethod("Reset", inParams, null);
				return (WbemError)Convert.ToUInt32(outParams.Properties["ReturnValue"].Value);
			}
			else {
				return WbemError.NoErr;
		...
		/// <summary>
		/// <para>Not implemented. For more information about how to implement this method, see the SetPowerState method in CIM_NetworkAdapter.</para>
		/// </summary>
		/// <param name="PowerState"></param>
		/// <param name="Time"></param>
		/// <returns></returns>
		public WbemError SetPowerState(ushort PowerState, DateTime Time)
		{
			if ((isEmbedded == false)) {
				ManagementBaseObject inParams = null;
				inParams = internalLateBoundObject.GetMethodParameters("SetPowerState");
				inParams["PowerState"] = ((ushort)(PowerState));
				inParams["Time"] = ToDmtfDateTime(((DateTime)(Time)));
				ManagementBaseObject outParams = internalLateBoundObject.InvokeMethod("SetPowerState", inParams, null);
				return (WbemError)Convert.ToUInt32(outParams.Properties["ReturnValue"].Value);
			}
			else {
				return WbemError.NoErr;
...
Der_Treiber_für_dieses_Gerät_ist_entweder_beschädigt_oder_es_stehen_nicht_genügend_Arbeitsspeicher_oder_andere_Ressourcen_zur_Verfügung_ = 3,
		...
		public class NetworkAdapterCollection : object, ICollection<NetworkAdapter>
		{
			#region internal Properties
			...
			#region Public Methods
			public virtual void CopyTo(NetworkAdapter[] array, int index) {
			...
			{
				return ((NetworkAdapterCollection)this).GetEnumerator();
			}
			public virtual IEnumerator<NetworkAdapter> GetEnumerator() {
			...
			public virtual void Add(NetworkAdapter nac)
			{
			}
			/// <summary>
			/// <para>Not working!</para>
			/// </summary>
			/// <param name="nac"></param>
			public virtual bool Remove(NetworkAdapter nac)
			{
				return false;
			}
			/// <summary>
			...
			/// <param name="nac"></param>
			/// <returns></returns>
			public virtual bool Contains(NetworkAdapter na)
			{
				foreach (NetworkAdapter na1 in this)
					if (na1 == na)
						return true;
				return false;
			}
			...
			#region IEnumerator
			public class NetworkAdapterEnumerator : object, IEnumerator<NetworkAdapter> {
				...
				object IEnumerator.Current
				{
					get
					{
						return ((NetworkAdapterEnumerator)this).Current;
					}
				}
				public virtual NetworkAdapter Current {
					get {
			...
			#region Operators
			public NetworkAdapter this[uint index]
			{
				get
				{
					if (index > this.Count)
					...
					IEnumerator<NetworkAdapter> enumerator = this.GetEnumerator();
					enumerator.Reset();
					uint i = 0;
					do
					{
